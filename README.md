...
- Scheduling: something must call `/api/cron/tick` with `Authorization: Bearer $CRON_SECRET`. Vercel Hobby allows a Cron Job only once a day, so `.github/workflows/jagr-cron.yml` calls it every 15 minutes (set the `JAGR_APP_URL` variable and `CRON_SECRET` secret in GitHub Actions). Missed slots are caught up, never lost.
