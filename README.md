...
Without a backend Jagr is browser-local. With one, workspaces live in Postgres, monitoring runs on a schedule (a scheduler calls `/api/cron/tick`) without a browser open, approvals are enforced on the server, and people sign in with Google or GitHub. See `.env.example` for the variables and `docs/ARCHITECTURE.md` for the design.
...
- Production: set `DATABASE_URL`, `JAGR_APP_URL`, `JAGR_SESSION_SECRET`, `JAGR_SECRET_KEY`, `CRON_SECRET` and a sign-in provider. Migrations run on start. Until they are set, `/api/*` answers 503 and the browser stays local. Step by step: `docs/DEPLOY.md`.
- Scheduling: something must call `/api/cron/tick` with `Authorization: Bearer $CRON_SECRET`. Vercel Hobby allows a Cron Job only once a day, so `.github/workflows/jagr-cron.yml` calls it every 15 minutes (set the `JAGR_APP_URL` variable and `CRON_SECRET` secret in GitHub Actions). Each tick enqueues the watch slots that fell due since the previous tick; after a gap only the latest missed slot per watch runs (older ones are skipped, not replayed). GitHub change monitoring reads 6 hours back on every run, so a late tick still sees the changes. `enqueued: 0` is normal when no slot was due. How to confirm it runs: `docs/DEPLOY.md` → *Verify scheduling*.
- `npm run smoke:production` checks a deployment (`JAGR_SMOKE_URL`) and, given a Postgres (`JAGR_SMOKE_DATABASE_URL`) and a GitHub token, runs the whole server lifecycle with GitHub live.
...
- Connectors (server only): Amplitude (metrics, annotations), GitHub (deployments, releases), Jira Cloud (issues, versions), Intercom (support conversations), and Slack for outbound alerts and briefs. See `docs/CONNECTORS.md`. `npm run eval:connectors` runs a read-only live check against the configured credentials; `npm run eval:dogfood` runs the whole single-tenant loop against them (see *Live verification* in `docs/CONNECTORS.md`).
- Server workspaces in the browser: Settings → *Where this workspace lives* → sign in → create or open a server workspace (live sources or imported files). Every page then shows that workspace — the same pages and rules as a browser workspace, with changes made through the server API. Sources → *Connect a source* takes an API key or token (stored encrypted on the server, never sent back). `npm run eval:ui` drives this path in Chromium.
