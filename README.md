...
- Connectors (server only): Amplitude (metrics, annotations), GitHub (deployments, releases), Jira Cloud (issues, versions), Intercom (support conversations), and Slack for outbound alerts and briefs. See `docs/CONNECTORS.md`. `npm run eval:connectors` runs a read-only live check against the configured credentials.
