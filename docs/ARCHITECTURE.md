...
| 5 Investigation replay | **Done** | `app/replay.ts`: the original investigation as product data — passes, frames (every one a recorded step or recorded decision, via the UI's `view/replay.ts`), the evidence snapshot and assumptions — built from storage only. `GET /api/workspaces/:id/investigations/:iid/replay[?pass=n]`. *Run again* is a separate operation (`POST …/rerun`, connected workspaces): a new pass over current data, appended; the original passes stay as recorded. Imported and sample data refuse it (their data does not change and a re-run would replace investigations). Tested with every provider down and the source disconnected: replay answers identically and makes no provider call. |
| 6 Morning brief | **Done** | The brief domain is `engine/brief.ts` (items by the investigations' own attention and each watch's brief level — no second ranking) plus the UI's `view/brief.ts`. New: briefs are stored (`Repositories.briefs`, memory + Postgres, contract-tested), carry the **changes** that are evidence in reported items (with timing kind), and are served as `GET /api/workspaces/:id/briefs/latest` and in the snapshot; the brief job passes stored in-app alerts so an item already sent is marked as such; the Slack brief is built from the same brief view (“Good morning. N things need your attention.”, What changed / found / recommended per item, uncertainty, quiet count, deep links). Quiet mornings still produce a brief. |
| 7 Evaluation Lab backend | **Done** | `app/evaluationLab.ts`: the golden, adversarial and planner-attack suites run through the real engine on their deterministic fixture nights (no network, no AI provider) and reported with the Evaluation Lab's own builders — seven dimensions (detection, evidence grounding, causality restraint, attention, deduplication, tool selection, approval enforcement) as the evaluators' counts, never a composite score; every scenario with expected / actual / status / checks; and coverage of the product scenarios (conversion drop after release, noise, source unavailable, duplicate incident, customer evidence only, planned release only, unsupported causality, multiple corroborating sources, approval enforcement). `GET /api/evaluations` (signed-in; computed once per server instance). No traces, rationales or model reasoning are exposed. |
| 8 Live connector verification | **Partial** — GitHub read path LIVE VERIFIED; Amplitude, Jira, Intercom, Slack NOT VERIFIED (no credentials) | See docs/CONNECTORS.md → *Re-verification (v1.2)*. GitHub was exercised through the new connection lifecycle API end to end (connect, test, read, links resolve, disconnect, no token stored). Token authentication cannot be proven in this environment (its egress proxy injects a GitHub credential). The other four were verified only on their real rejection paths. |
| 9 Production hardening | **Done** | Audit of isolation, leakage, audit logging, approvals, retries, idempotency, notifications, stale/outage handling, leases and the scheduler. Fixed: (1) **Slack duplicate delivery** — deliveries now claim the (channel, dedupe key) atomically *before* sending and settle after; only the claimer sends; a failure releases the key; a claim interrupted mid-send is retried after 15 min and logged as "delivery unknown" (a rare duplicate beats a lost alert). (2) **Concurrent runs** — one monitoring run per workspace (`Repositories.locks`, atomic, TTL 15 min); a second run gets *busy* (API 409; a queued job retries in a minute). (3) **Audit entries dropped** — run and connection audit ids collided when several watches ran at the same due time; ids are now unique. (4) **Job leases** — a job whose lease expired while earlier jobs in the batch ran is renewed if nobody took it over (lease ownership is the token), skipped if another worker did; settling a taken-over job no longer crashes the whole drain. (5) Server error logs are redacted. Verified unchanged: memberships re-read per request (revocation is immediate), every workspace route membership-scoped, approvals enforced on the server (403 / 409), CSRF on every state change, httpOnly/Secure/SameSite=Lax session cookie, no credentials in responses/exports/audit, AI prompts redacted and `aiEgressAllowed` enforced, providers' outages recorded as gaps, scheduler limited to active watches of connected workspaces. |

### Final V1.2 integration regression

- **My-data feedback contradiction — fixed (engine).** In My-data mode the imported feedback channel reuses a store's source id and inherited the store's capabilities, so the checkout investigation reported "No crash free sessions ios data … not checked" and "no releases" *from the feedback channel* beside the four negative reviews it did read. Neither role was ever imported into it (crash metrics are not even importable in V1). The imported world now declares each channel's capabilities from what was imported (`World.capabilities`); the feedback channel serves feedback only. Regression test in `evidence/chain.test.ts` (fails without the fix). Verdict lock and evaluation suites unchanged.
- **Two "stale" definitions — kept distinct, deliberately.** *Engine / investigation* (`roles/types.ts healthOf`): a source is stale for a read when its data stops before the time that read needs — exact, because a missing minute inside an investigation window must be reported as not seen. *Server connection health* (`connections/model.ts STALE_AFTER_MS`, 6 h): is this connection's sync healthy — tolerant of normal provider lag, so a health badge does not flap. Unifying them would either let investigations accept missing data silently or mark every lagging connection stale; live connectors also do not report `freshAsOf` yet. They answer different questions and stay separate.
- **Known pre-existing issue:** at 390 px, /investigations and /signals overflow horizontally by 4 px because of a Demo Night control row ("Run Overnight · Reset & replay Demo night"). Present before V1.2 (measured on 8418358); not changed here (Demo Night is out of scope). `npm run eval:qa` reports it as known.
