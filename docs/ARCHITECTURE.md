...
- **Notification channels are opaque ids** (`'in_app'`, later `'slack'`) so no port type names a vendor.
...
- **Evidence links for connected sources use the provider's base URL** (`PROVIDERS.externalBase`), not the record's own deep link: records carry `provenance.url`, but the evidence-link builder does not read it yet. Debt, not hidden.
