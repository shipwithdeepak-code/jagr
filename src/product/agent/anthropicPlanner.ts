...
export function createAnthropicPlannerClient(cfg: { apiKey: string; model?: string; baseUrl?: string; fetch?: Fetch }) {
  const adapter = createAnthropicAdapter({ provider: 'anthropic', model: cfg.model ?? ANTHROPIC_DEFAULT_MODEL, apiKey: cfg.apiKey, baseUrl: cfg.baseUrl }, cfg.fetch);
  return {
    model: adapter.model,
