...
export interface SourceViewOptions {
  ...
  /**
   * Server workspaces: the viewer may change connections (owner / admin). Then connect, reconnect and
   * disconnect map to the connection API — except for connections the deployment environment manages.
   */
  manage?: boolean;
}

const NO_OAUTH = 'Live sources connect in a server workspace (sign in under Settings), with an API key or token. Connecting by provider sign-in (OAuth) isn’t built.';
const NO_ENDPOINT = 'This workspace lives in your browser. Connections are managed in a server workspace.';
const ENV_MANAGED = 'This connection’s credentials come from the deployment environment; change them there.';
const NOT_ADMIN = 'Only workspace owners and admins can change connections.';
...
function actionsFor(group: SourceGroup, view: ConnectionView | undefined, manage = false): SourceAction[] {
  ...
  // Changes need a server connection the workspace itself manages, and a member allowed to change it.
  const change = !view ? { ok: false, why: NO_ENDPOINT } : view.managedBy === 'environment' ? { ok: false, why: ENV_MANAGED } : !manage ? { ok: false, why: NOT_ADMIN } : { ok: true, why: '' };
  const reconnect: SourceAction = change.ok ? { id: 'reconnect', label: 'Reconnect', available: true, reason: 'Enter a new credential; it replaces the stored one and is tested at once.' } : { id: 'reconnect', label: 'Reconnect', available: false, reason: change.why };
  const disconnect: SourceAction = change.ok ? { id: 'disconnect', label: 'Disconnect', available: true, reason: 'Deletes the stored credential. Investigations then record this source as not configured.' } : { id: 'disconnect', label: 'Disconnect', available: false, reason: change.why };
  switch (group) {
    case 'connected':
    case 'stale':
    case 'unavailable':
    ...
    case 'not_configured':
      return [change.ok ? { id: 'connect', label: 'Connect', available: true, reason: 'Enter an API key or token; it is stored encrypted on the server and tested at once.' } : { id: 'connect', label: 'Connect', available: false, reason: view ? change.why : NO_OAUTH }];
    case 'imported':
    case 'simulated':
      return [];
  }
...
export function sourceViews(connections: SourceConnection[], opts: SourceViewOptions): SourceView[] {
  ...
  for (const conn of connections) {
    ...
    views.push({
      ...
      actions: conn.state === 'simulated' || conn.state === 'imported' ? [] : actionsFor(group, view, opts.manage),
    });
  }
