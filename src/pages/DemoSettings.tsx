...
export function DemoSettingsPage() {
  ...
  return (
    <>
      <PageHeader title="Demo night settings" description="How the scripted replay behaves: what its agent watches, who owns what, how much it may do on its own and when it interrupts people. These settings apply to Demo night only — your workspace is configured in Settings." />

      <div className="space-y-6">
        <Panel title="What the agent watches" hint="Thresholds are the relative move (in the bad direction) required before a signal can be anomalous.">
        ...
        <Panel title="Who owns what" hint="The agent files work to the team that owns the implicated component — not the metric that moved.">
          <div className="grid gap-3 sm:grid-cols-2">
            {s.owners.map((o) => (
              <div key={o.area} className="flex items-center justify-between gap-3 rounded-lg border border-line px-3 py-2">
                <span className="text-[14px] font-medium">{AREA_LABELS[o.area]}</span>
        ...
        <Panel title="Autonomy policy" hint="Each level requires the ones below it. Turning one off disables everything above.">
          ...
          <div className="mt-4 grid grid-cols-5 gap-1 text-center text-[12px]">
            {AUTONOMY_LEVELS.map((l) => (
              <div key={l.level} className="rounded bg-subtle px-1 py-1.5" title={l.description}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {(['critical', 'high', 'medium', 'low'] as AlertSeverity[]).map((sev) => (
              <div key={sev} className="flex items-center justify-between gap-3 rounded-lg border border-line px-3 py-2">
                <span className="text-[14px] font-medium capitalize">{sev}</span>
        ...
        <Panel title="Schedule">
          ...
          <div className="mt-4 flex items-center justify-between gap-3 rounded-lg border border-line px-3 py-2.5">
            <div>
              <div className="text-[14px] font-medium">Critical escalation</div>
              <div className="text-[12px] text-ink-3">Allow the agent to notify on-call during the night for critical findings.</div>
            </div>
